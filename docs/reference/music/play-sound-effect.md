...
You program can wait for the sound to finish before it runs its next step. To do this, set the play mode to `until done`. Otherwise, use `background` for the program to continue immediately after the sound starts.
...
* **sound**: a [string]() the sound that generated by a sound expression.
