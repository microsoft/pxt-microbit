...
7. At the end of a set of rounds (or end of specified time), the total score for both players should be recorded on paper. If the third @boardname@ was programmed to keep score, Player 3 should display the scores from the @boardname@ and write them down on a score sheet. This sheet would typically have the players names on it, not player numbers since students can switch positions.

If time permits, students should switch positions (e.g., Player 1 becomes Player 2, Player 2 becomes Player 3, and Player 3 becomes Player 1) and repeat for the same amount of time/number of rounds until all students have had the opportunity to be Player 3. If time does not permit, play the game again on another day, switching positions.

## Variations

1. If students need to play in groups of 4, Player 3 would say “Salute!” and the sum or product of the two cards, as usual. Player 4 would be the score keeper and referee on any ties OR have an additional @boardname@ with a number displayed and students need to calculate using all three @boardname@s with numbers displayed.
2. Older students could create this game for younger students making sure to use an appropriate number range for the grade level. The older students or the older and younger students together could make some kind of headband to hold the @boardname@ on the forehead. The older students would then teach the younger students how to play the game.
